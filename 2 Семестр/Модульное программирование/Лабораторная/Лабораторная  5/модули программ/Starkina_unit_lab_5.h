...
if (p!=NULL) {
    do {
        if ((strcmp(p->city,city_in)==0)) {return p;}
        p = p->next;
    } while (p!=NULL);
    return NULL;
} else {
...
directory_elem* search_elem(directory_elem *head, int index_in) {
directory_elem *p;
p = head;
if (p!=NULL) {
    do {
        if ((p->index == index_in)) {return p;}
        p = p->next;
    } while (p!=NULL);
    return NULL;
} else {
...
if (name_file == "\n") {
    ...
    else {std::cout << "Довiдник порожнiй." << std::endl;}
} else {
...
} else { std::cout << "Помилка!! Файл не вiдкрився." << std::endl;}
} else {out_directory << "Довiдник порожнiй." <<  std::endl;}
out_directory.close();
}
}
